...
async def shiny_loop_buttons(controller_state: ControllerState):
    """
    ...
    base_shiny_probability = 0.0002441406
    for x in range(10000):
        ...
        time.sleep(1)
        await button_push(controller_state, 'a')
        print("Game started... ",end='', flush=True)
        # Game loading to credits, mash a after 20s
        time.sleep(20)
        await button_push(controller_state, 'a')
        time.sleep(1)
        await button_push(controller_state, 'a')
        ...
        time.sleep(1)
        await button_push(controller_state, 'a')
        print("Finished credits... ",end='', flush=True)
        #game loading...
        time.sleep(9)
        print("In game... Talking to pokemon...", flush=True)
        await button_push(controller_state, 'up', sec=2)
        time.sleep(1)
        await button_push(controller_state, 'a')
        time.sleep(1)
        await button_push(controller_state, 'a')
        ...
        user_input = asyncio.ensure_future(
            ainput(prompt='Loop done, is it shiny? Count: ' + str(counter) + ' Probability: ' + str(1 - math.pow((1-base_shiny_probability), counter)))
        )
...
def _register_commands_with_controller_state(controller_state, cli):
    ...
    async def shiny_loop():
        await shiny_loop_buttons(controller_state)

    cli.add_command(shiny_loop.__name__, shiny_loop)
